...
func (r Running) Calories() float64 {
    normalisedSpeed :=
        (CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) / MInKm

    return normalisedSpeed * r.Weight * r.Duration.Hours() * MinInHours
}
...
func (w Walking) Calories() float64 {
    if floatEqual(w.Height, 0.0) {
        return 0
    }

    normalisedSpeed := math.Pow(w.meanSpeed()*KmHInMsec, 2) *
        CaloriesSpeedHeightMultiplier * w.Weight / (w.Height / CmInM)

    return (CaloriesWeightMultiplier*w.Weight +
        normalisedSpeed) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
    if floatEqual(s.Duration.Hours(), 0.0) {
        return 0
    }
...
func (s Swimming) Calories() float64 {
    normalisedSpeed := s.meanSpeed() + SwimmingCaloriesMeanSpeedShift
    return normalisedSpeed * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
}
